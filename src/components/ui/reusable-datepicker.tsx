...
export const ReusableDatePicker = forwardRef<HTMLInputElement, DatePickerProps>(
  (
    ...
    // FIXED: Handle internal value and external value coordination
    // This effect now properly responds to external value changes (like form.reset())
    useEffect(() => {
      const normalizedExternalValue = normalizeToDate(value, dateFormat);
      const normalizedDefaultValue = normalizeToDate(defaultValue, dateFormat);
      
      // If there's an external value prop, always sync to it (including undefined/null for reset)
      if (value !== undefined) {
        setInternalValue(normalizedExternalValue);
        
        // Update input value and other state based on external value
        if (normalizedExternalValue) {
          ...
          // External value is null/undefined (form reset case)
          setInputValue("");
          setLastValidValue("");
        }
      } else if (normalizedDefaultValue && internalValue === undefined) {
        // Only set default value if no external value and no internal value
    ...
    // REMOVED: The old useEffect that was updating input value
    // The above useEffect now handles all value synchronization
