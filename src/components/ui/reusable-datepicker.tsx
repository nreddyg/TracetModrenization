...
//included year and month no design
...
//   const yearRange = Array.from({ length: 201 }, (_, i) => currentYear - 100 + i);
...
//     <div className="py-1">
//       {/* Header */}
//       <div className="flex items-center justify-between mb-4 px-3 gap-2">
//         <button
//           type="button"
//           onClick={goToPreviousMonth}
...
//             className="text-sm font-medium border border-gray-300 rounded px-2 py-1 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-blue-500 cursor-pointer"
//             title="Select month"
//           >
...
//             Now with quick month and year selection dropdowns
//           </p>
...
//                 tooltip="Use dropdowns to quickly jump to any month/year"
//                 allowClear
//                 showTodayButton
//               />
//               {selectedDate && (
...
//                 format="DD/MM/YYYY"
//                 maxDate={new Date()}
//                 tooltip="Easily select past dates using the year dropdown"
//                 required
//                 size="md"
//               />
//               {birthDate && (
...
//                 format="MM/DD/YYYY"
//                 minDate={new Date()}
//                 tooltip="Quickly navigate to future years"
//                 size="lg"
//               />
//               {futureDate && (
//                 <p className="text-sm text-gray-600 mt-2">
//                   Event Date: {futureDate.toLocaleDateString()}
//                 </p>
//               )}
//             </div>
...
//               <h3 className="font-semibold text-gray-800 mb-3">✨ New Features:</h3>
//               <ul className="space-y-2 text-sm text-gray-700">
//                 <li className="flex items-start">
//                   <span className="text-blue-600 mr-2">•</span>
//                   <span><strong>Month Dropdown:</strong> Quickly select any month without clicking arrows</span>
//                 </li>
//                 <li className="flex items-start">
//                   <span className="text-blue-600 mr-2">•</span>
//                   <span><strong>Year Dropdown:</strong> Jump to any year (±100 years range) instantly</span>
//                 </li>
//                 <li className="flex items-start">
//                   <span className="text-blue-600 mr-2">•</span>
//                   <span><strong>Arrow Navigation:</strong> Still available for quick month-by-month browsing</span>
//                 </li>
//                 <li className="flex items-start">
//                   <span className="text-blue-600 mr-2">•</span>
//                   <span><strong>All Features Preserved:</strong> Min/max dates, disabled dates, formats, and more</span>
//                 </li>
//               </ul>
//             </div>
//           </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="relative" ref={monthDropdownRef}>
            <button
              ...
              className="text-sm font-medium hover:bg-gray-100 px-2 py-1 rounded-md transition-colors"
            >
                ...
                {monthNames.map((name, index) => (
                  <button
                    ...
                    className={cn(
                      ...
                      index === month.getMonth() && "bg-blue-50 text-blue-600 font-medium"
                    )}
                  >
                    {name}
                  </button>
          ...
          <div className="relative" ref={yearDropdownRef}>
            ...
            {showYearDropdown && (
              <div className="absolute top-full left-0 mt-1 bg-white border border-gray-300 rounded-md shadow-lg z-50 overflow-y-auto"
                   style={{ maxHeight: '280px', minWidth: '80px' }}>
                {yearRange.map((year) => (
                  <button
                    key={year}
                    type="button"
                    ...
                    className={cn(
                      "w-full text-left px-3 py-2 text-sm hover:bg-gray-100 transition-colors",
                      year === month.getFullYear() && "bg-blue-50 text-blue-600 font-medium"
                    )}
                  >
                    {year}
                  </button>
...
export const ReusableDatePicker = forwardRef<HTMLInputElement, DatePickerProps>(
  (
    ...
    // Smart position calculation with responsive maxHeight
    const calculatePopupPosition = (): PopupPosition => {
      if (!datePickerRef.current) {
        return { top: 0, left: 0, width: 0, showAbove: false };
      }
      ...
      const popupWidth = Math.max(320, inputRect.width);
      const margin = 16;
      const spacing = 4;
    ...
    // Handle internal value and external value coordination
    useEffect(() => {
      const normalizedExternalValue = normalizeToDate(value, dateFormat);
      const normalizedDefaultValue = normalizeToDate(defaultValue, dateFormat);
      
      if (value !== undefined) {
        setInternalValue(normalizedExternalValue);
        
        if (normalizedExternalValue) {
          const formattedValue = formatDate(normalizedExternalValue, dateFormat);
          setInputValue(formattedValue);
          setLastValidValue(formattedValue);
          setMonth(normalizedExternalValue);
        } else {
          setInputValue("");
          setLastValidValue("");
        }
        ...
        setInternalValue(normalizedDefaultValue);
        const formattedValue = formatDate(normalizedDefaultValue, dateFormat);
        setInputValue(formattedValue);
        setLastValidValue(formattedValue);
        setMonth(normalizedDefaultValue);
      }
    }, [value, defaultValue, dateFormat]);
//v2
// const handlePopupClose = () => {
...
//       // Don't revert if the input already matches a valid internal value
//       if (internalValue && inputValue === formatDate(internalValue, dateFormat)) {
//         return;
//       }
...
const isDateDisabled = (date: Date): boolean => {
    ...
    const validateAndCorrectInput = useCallback(() => {
      ...
      const parsed = parseDate(trimmedValue, dateFormat);
      ...
      // Check if the parsed date is disabled
      const isDisabled = 
        (minDate && parsed < minDate) ||
        (maxDate && parsed > maxDate) ||
        (Array.isArray(disabledDates) && disabledDates.some((d) => isSameDay(d, parsed))) ||
        (typeof disabledDates === "function" && disabledDates(parsed));
      
      if (isDisabled) {
        // Disabled date - clear everything
        setInputValue("");
        setInternalValue(undefined);
        onChange?.(undefined);
        setLastValidValue("");
      ...
      // Valid date - format it properly and accept it
      const formattedValue = formatDate(parsed, dateFormat);
      setInputValue(formattedValue);
      setInternalValue(parsed);
      onChange?.(parsed);
    ...
    }, [dateFormat, onChange, minDate, maxDate, disabledDates]);

    const handleInputBlur = (e: React.FocusEvent<HTMLInputElement>) => {
      // Don't validate if we're clicking inside the calendar
      const relatedTarget = e.relatedTarget as HTMLElement;
      if (calendarRef.current?.contains(relatedTarget)) {
        return;
      }
      
      // If clicking on the wrapper/icon to open calendar, don't validate yet
      if (datePickerRef.current?.contains(relatedTarget)) {
        return;
      }
      
      // If popup is open, don't validate on blur - wait for popup close
      if (actualOpen) {
        return;
      }
