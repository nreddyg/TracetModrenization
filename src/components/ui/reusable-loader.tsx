...
export interface ReusableLoaderProps {
  ...
  strokeColor?: string;
  trailColor?: string;
  type?: 'loader' | 'default' | 'dots' | 'pulse' | 'bars' | 'progress' | 'spin' | 'bouncing';
  direction?: 'horizontal' | 'vertical';
  bouncingColor?: string;
}
...
export const ReusableLoader: React.FC<ReusableLoaderProps> = ({
  ...
  const BouncingSpinner = () => {
    ...
    return (
      <>
        <style>{`
          @keyframes bouncing-loading {
            to {
              opacity: 0.3;
              transform: translateY(-${ballSize * 2.5}px);
            }
          }
          .bouncing-ball {
            animation: bouncing-loading 0.9s infinite alternate;
          }
        `}</style>
        <div className="flex gap-1">
          {[0, 1, 2].map((i) => (
            <div
              key={i}
              className="bouncing-ball rounded-full"
              style={{
                width: ballSize,
                height: ballSize,
  ...
  const renderSpinner = () => {
    ...
    switch (type) {
      ...
      case 'bouncing': return <BouncingSpinner />;
      case 'default': return <DefaultSpinner />;
      default: return <BouncingSpinner />;
    }
  };
