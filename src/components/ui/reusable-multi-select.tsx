    ...
    const renderSelectedTags = () => {
      ...
      return (
        <div className="flex flex-wrap gap-1">
          ...
          {remainingCount > 0 && (
            <span className={cn(
              "inline-flex items-center px-0 py-1 rounded-md text-xs bg-gray-100 text-gray-600",
              size === 'small' && "px-0 py-0.5",
              size === 'large' && "px-0 py-1.5 text-sm"
            )}>
              +{remainingCount} more
            </span>
          )}
      ...
      const baseContent = (
        <div className="flex flex-col h-full">
          {loading ? (
            ...
            <div className="px-3 py-8 text-center text-gray-500 truncate" title={searchTerm}>
        ...
        if (groupedOptions) {
          groupedOptions.forEach(group => {
            ...
            if (groupFilteredOptions.length > 0) {
              actualContentHeight += groupHeaderHeight;
              if (groupSelectAll) actualContentHeight += 36;
            ...
            <div className="flex items-center flex-1 min-w-0">
              ...
              {searchable ? (
                <input
                  ...
                  disabled={disabled}
                  className={cn(
                    "flex-1 bg-transparent outline-none text-sm px-3 py-2",
                    disabled && "cursor-not-allowed",
                    normalizedValue.length > 0 ? "min-w-[20px] px-1" : "w-full px-3"
                  )}
                />
              ) : (
