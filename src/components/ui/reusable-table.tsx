...
const exportToExcel = async (exportData: any[], table: TanstackTable<any>) => {
  ...
  const Pagination = ({ table }: { table: TanstackTable<any> }) => {
    ...
    return (
      <div className="flex items-center justify-between flex-wrap sm:flex-nowrap px-2 py-3 bg-white text-sm rounded-b-xl gap-1">
  ...
  export function ReusableTable<T = any>({
    ...
    return (
      <div className={cn("space-y-2 ", className)}>
        ...
        {/* Controls */}
        <div className="flex flex-wrap px-3 py-2 items-center gap-4">
          {enableSearch && (
          ...
          {onRefresh && (
                <Button variant="outline" size="sm" onClick={onRefresh} className="flex-1 sm:flex-none  text-slate-700" >
                  <RefreshCw className="w-4 h-4 mr-2 font-light" />
                  Refresh
                </Button>
              )}
        ...
        {/* Table */}
        <div className={cn("bg-white shadow-sm border-y border-gray-200 overflow-hidden",containerClassName)}>
            <ScrollArea horizontal className='w-full pb-2'>
            <table ref={tableRef} className="w-full min-w-max border-collapse text-sm text-[#71717A] bg-[#f9f9fa]">
              <thead className="bg-white text-[#71717A] uppercase text-sm font-semibold tracking-wide bg-[#f9f9fa]">
                      ...
                      >
                        ...
                        {header.column.getCanResize() && (
                          <div
                            ...
                            className="absolute right-0 top-0 h-full w-[0.05rem] cursor-col-resize group z-30"
                            style={{
                              transform: "", // ensures correct alignment
                        ...
                        {row.getVisibleCells().map(cell => {
                          ...
                          return (
                            <td
                              key={cell.id}
                              className={cn(
                                "px-4 py-3 text-black border-b border-gray-200 ",
                                rowHeightClasses[rowHeight]
                              )}
                              style={{
