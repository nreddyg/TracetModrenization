...
const SelectionTag: React.FC<{
    ...
    return (
        <span className="inline-flex items-center px-2 py-1 mr-1 mb-1 text-xs font-medium bg-blue-100 text-blue-800 rounded-md">
            <span title={title}>{displayTitle}</span>
            {!disabled && (
                <button
                    className="ml-1 inline-flex items-center justify-center w-3 h-3 text-blue-600 hover:text-blue-800"
...
export const TracetTreeSelect: React.FC<TracetTreeSelectProps> = ({
    ...
    // Render selected tags (similar to MultiSelect)
    const renderSelectedTags = () => {
        const maxTags = multiSelectConfig.maxTagsCount || 1;
        const maxTextLen = multiSelectConfig.maxTagTextLen || 20;
        ...
        if (multiSelectConfig.multiple && multiSelectConfig.treeCheckable) {
            ...
            if (selectedValues.length <= maxTags) {
                ...
                return (
                    <div className="flex flex-wrap items-center">
                        ...
                        {remainingCount > 0 && (
                            <span className="inline-flex items-center px-2 py-1 mr-1 mb-1 text-xs font-medium bg-gray-100 text-gray-600 rounded-md">
                                +{remainingCount} more
                            </span>
                        )}
                    </div>
            ...
            // Single selection or comma-separated display
            const displayTexts = selectedValues.slice(0, maxTags).map(val => {
                const node = findNodeByValue(treeData, val);
                const title = node?.title || val.toString();
                return title.length > maxTextLen ? `${title.slice(0, maxTextLen)}...` : title;
            });
            ...
            return (
                <div className="flex items-center">
                    ...
                    {remainingCount > 0 && (
                        <span className="ml-1 text-xs text-gray-600">+{remainingCount} more</span>
                    )}
                </div>
            );
        }
    ...
    // Render dropdown content - Fixed double scrollbar issue
    const renderDropdownContent = () => {
        return (
            <>
                {/* Search (only show if inline search is disabled) */}
                ...
                {/* Tree Content - Removed extra padding and overflow */}
                <div className="py-1">
                    {filteredTreeData.length > 0 ? (
                        renderTreeNodes(filteredTreeData)
                    ) : (
    ...
    // Render popup with improved positioning (same as MultiSelect)
    const renderPopup = () => {
        const inputRect = containerRef.current?.getBoundingClientRect();
        if (!inputRect) return null;
        ...
        return (
            ...
            >
                {/* Single overflow container to prevent double scrollbar */}
                <div
                    className="overflow-y-auto overflow-x-hidden"
                    style={{ maxHeight: `${maxHeight}px` }}
                >
                    ...
                    <div className="flex-1 min-w-0 px-3 py-2">
                        {isSearching && showSearch ? (
                            <input
                                ...
                                disabled={disabled}
                                autoComplete="off"
                                style={{ minWidth: isSearching || selectedValues.length === 0 ? '120px' : '60px' }}
                            />
                        ) : (
                            selectedValues.length === 0 ? (
                ...
                {isOpen && !disabled && (
                    <>
                        {usePortal ? (
                            ...
                            >
                                <div className="bg-white border border-gray-300 rounded-md shadow-lg overflow-y-auto">
                                    {renderDropdownContent()}
                                </div>
                            </div>
                        )}
