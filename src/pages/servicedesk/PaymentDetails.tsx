...
//   // Watch for changes in AMCFromDate, Type, and AMCToDate
...
console.log(data,"data1554")
  ...
  // // Effect to handle automatic AMCToDate calculation and clearing
  // useEffect(() => {
  //   if (watchAMCFromDate && watchType) {
  //     calculateAMCToDate(watchAMCFromDate, watchType);
  //   } else if (!watchAMCFromDate) {
  //     // Clear AMCToDate and AMCExpiryDate when AMCFromDate is cleared
  //     setValue("AMCToDate", "", { shouldValidate: true });
  //     setValue("AMCExpiryDate", "", { shouldValidate: true });
  //   }
  // }, [watchAMCFromDate, watchType, setValue]);
  ...
  useEffect(() => {
    if (subData) {
     ...
     setFields((prev) =>
        prev.map((f) => {
          if (f.name === "Type" || f.name==='AMCFromDate' || f.name==='AMCToDate' || f.name==='AMCPaidDate' ) {
