import { URL_DELETE_CONVERSION,URL_DELETE_UNITS_OF_MEASURE, URL_GET_MANAGE_UNITS_OF_MEASURE_DATA, URL_GET_UNITS_OF_MEASURE_DATA, URL_POST_NEW_UNITS_OF_MEASURE, URL_POST_UPDATE_UNITS_OF_MEASURE } from '@/config/apiUrls';
import api from './api';
...
export const getUOMData = async (CompId: string): Promise<APIResponse<any>> => {
    try {
...
export const getEditUOMData = async (CompId: string,id:number | string): Promise<APIResponse<any>> => {
    try {
        const response = await api.get(URL_GET_UNITS_OF_MEASURE_DATA, { params: { CompId: CompId,UOMId:id } })
        return {success: true,data: response.data,}
    } catch (err: any) {
        return {success: false,message: err.response?.data?.message || err.message,status: err.response?.status};
    }
